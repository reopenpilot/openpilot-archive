...
class CarController(CarControllerBase):
  ...
  def update_pedal(self, CC, CS, now_nanos, frogpilot_toggles):
    actuators = CC.actuators
    hud_control = CC.hudControl
    pcm_cancel_cmd = CC.cruiseControl.cancel
    lat_active = CC.latActive and abs(CS.out.steeringTorque) < MAX_USER_TORQUE
    ...
    if frogpilot_toggles.sport_plus:
      pcm_accel_cmd = clip(actuators.accel, self.params.ACCEL_MIN, min(frogpilot_toggles.max_desired_acceleration, get_max_allowed_accel(CS.out.vEgo)))
    else:
      pcm_accel_cmd = clip(actuators.accel, self.params.ACCEL_MIN, min(frogpilot_toggles.max_desired_acceleration, self.params.ACCEL_MAX))
    ...
    if (self.frame % 3 == 0 and self.CP.openpilotLongitudinalControl) or pcm_cancel_cmd:
      ...
      if pcm_cancel_cmd and self.CP.carFingerprint in UNSUPPORTED_DSU_CAR:
        ...
        can_sends.append(toyotacan.create_accel_command(self.packer, pcm_accel_cmd, pcm_cancel_cmd, True, self.standstill_req, lead, CS.acc_type, fcw_alert,
                                                        self.distance_button, self.reverse_cruise_active))
        self.accel = pcm_accel_cmd
      else:
