...
class LongControl:
  ...
  def update_old_long(self, active, CS, long_plan, accel_limits, t_since_plan, frogpilot_toggles):
    ...
    if len(speeds) == CONTROL_N:
      ...
      actuator_delay = frogpilot_toggles.longitudinalActuatorDelay
      v_target = interp(actuator_delay + t_since_plan, CONTROL_N_T_IDX, speeds)
      a_target = a_target_now if actuator_delay == 0 else 2 * (v_target - v_target_now) / actuator_delay - a_target_now

      v_target_1sec = interp(actuator_delay + t_since_plan + 1.0, CONTROL_N_T_IDX, speeds)
    else:
      v_target = 0.0
      v_target_now = 0.0
      v_target_1sec = 0.0
