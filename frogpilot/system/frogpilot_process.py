...
from openpilot.frogpilot.selfdrive.modeld.model_manager import MODEL_DOWNLOAD_ALL_PARAM, MODEL_DOWNLOAD_PARAM, ModelManager
from openpilot.frogpilot.selfdrive.ui.theme_manager import THEME_COMPONENT_PARAMS, ThemeManager
from openpilot.frogpilot.common.frogpilot_api import FrogPilotAPI
from openpilot.frogpilot.system.manager.frogpilot_functions import backup_toggles
from openpilot.frogpilot.common.frogpilot_utilities import is_url_pingable, run_thread_with_lock
from openpilot.frogpilot.selfdrive.car.toyota.door_control import lock_doors
from openpilot.frogpilot.selfdrive.navd.map_scheduler import update_maps
from openpilot.frogpilot.selfdrive.pandad.panda_flashing import flash_panda
from openpilot.frogpilot.system.reporting import capture_report
from openpilot.frogpilot.system.updated.update_helpers import update_openpilot
...
from openpilot.frogpilot.selfdrive.controls.lib.frogpilot_planner import FrogPilotPlanner
from openpilot.frogpilot.system.frogpilot_stats import send_stats
from openpilot.frogpilot.system.frogpilot_tracking import FrogPilotTracking

ASSET_CHECK_RATE = (1 / DT_MDL)

def assets_checks(model_manager, theme_manager, frogpilot_toggles, frogpilot_api):
  ...
  if report_data:
    run_thread_with_lock("capture_report", capture_report, (report_data["DiscordUser"], report_data["Issue"], dict(vars(frogpilot_toggles)), frogpilot_api))
...
def frogpilot_thread():
  ...
  while True:
    ...
    if started and not started_previously:
      if error_log.is_file():
        error_log.unlink()
    ...
    elif not started and started_previously:
      ...
      if time_validated:
        run_thread_with_lock("send_stats", send_stats, (params, frogpilot_toggles, frogpilot_api))
    ...
    if rate_keeper.frame % ASSET_CHECK_RATE == 0:
      assets_checks(model_manager, theme_manager, frogpilot_toggles, frogpilot_api)
