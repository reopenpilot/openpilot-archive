...
from openpilot.frogpilot.selfdrive.controls.lib.frogpilot_following import FrogPilotFollowing
from openpilot.frogpilot.selfdrive.controls.lib.frogpilot_vcruise import FrogPilotVCruise
from openpilot.frogpilot.selfdrive.controls.lib.weather_checker import WeatherChecker
...
class FrogPilotPlanner:
  ...
  def update(self, now, time_validated, sm, frogpilot_toggles):
    ...
    if sm["controlsState"].enabled and frogpilot_toggles.conditional_experimental_mode:
      ...
      self.cem.slow_lead_detected = False

      self.cem.curvature_filter.x = 0
      self.cem.slow_lead_filter.x = 0
    ...
    if sm.alive["liveLocationKalman"] and sm["liveLocationKalman"].gpsOK and localizer_valid:
      gps_position = {
