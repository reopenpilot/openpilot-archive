...
class FrogPilotCard:
  ...
  def update(self, carState, frogpilotCarState, sm):
    ...
    self.always_on_lateral_enabled &= not any(event.immediateDisable for events in sm["onroadEvents"] for event in events if event.name != EventName.speedTooLow) or self.car.frogpilot_toggles.frogs_go_moo
